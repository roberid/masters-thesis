...
#ax.set_xticks([])
#ax.set_yticks([])
#ax.set_ylabel(r'$\mathrm{Density}$', fontsize=24, labelpad=35)
#ax.set_xlabel(r'$\Delta c z / \sigma$', fontsize=24, labelpad=25)
...
ax0.set_xlabel(r'$\Delta c z / \sigma$', fontsize=24)
ax0.set_ylabel(r'$\mathrm{Density}$', fontsize=24)
ax0.tick_params(axis='both', labelsize=16)

ax0.plot(x, y946, color='#8f5e99', ls='-', lw=lw)
ax0.plot(x, norm.pdf(x), color='#444444', ls='--', lw=lw)
ax0.fill_between(x, y946_up, y946_down, color='#8f5e99', lw=0, alpha=alph)

#ax0.text(-2.8, 0.35, 'NG', fontsize=20)
#ax0.text(2.3, 0.37, '(b)', fontsize=16)
...
ax1.set_xlabel(r'$\Delta c z / \sigma$', fontsize=24)
ax1.set_ylabel(r'$\mathrm{Density}$', fontsize=24)
ax1.tick_params(axis='both', labelsize=16)

ax1.plot(x, y185, color='#33825e', ls='-', lw=lw)
ax1.plot(x, norm.pdf(x), color='#444444', ls='--', lw=lw)
ax1.fill_between(x, y185_up, y185_down, color='#33825e', lw=0, alpha=alph)

#ax1.text(-2.8, 0.35, 'G', fontsize=20)
#ax1.text(2.3, 0.37, '(a)', fontsize=16)
#ax1.text(-1, 0.1, 'Gaussian', fontsize=10)

fig.savefig('vdist_ver3.png', dpi=600, bbox_inches='tight')

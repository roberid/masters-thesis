...
def f(x, a0, mu0, sig0, a1, mu1, sig1, a2, mu2, sig2):
    return a0 * norm(x, mu0, sig0) + a1 * norm(x, mu1, sig1) + a2 * norm(x, mu2, sig2)

fit = curve_fit(f, x, y, p0 = [1,0.4,0.2,1,0.6,0.1,2,0.8,0.1])
...
ax0.plot(x, f(x,fit[0][0],fit[0][1],fit[0][2],fit[0][3],fit[0][4],fit[0][5],fit[0][6],fit[0][7],fit[0][8]), 'k-', lw=lw+1)
...
ax0.plot(x, fit[0][6] * norm(x,fit[0][7],fit[0][8]), color='g', ls='-', lw=lw)
...
fig.savefig('gr_dist_gv.pdf', dpi=1200, bbox_inches='tight')
